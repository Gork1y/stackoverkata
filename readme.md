...
Чтобы не вносить свои данные связанные с базой данных в properties, можно с помощью конфигурации проекта
задавать свои значения из определенного профиля. У нас это будут профили **local** и **dev**.

Для начала, нужно зайти в конфигурацию проекта и создать свои переменные среды (Environment variables) которые будут создержать в себе данные о подключении к базе данных.

Затем, необходимо выполнить следующие действия:
1. Создать файл с **local** конфигураций и назвать его "application-local.properties" и заполнить его своими значениями (строки подключения к базе данных и т.п)
2. Создать файл с **dev** конфигураций и назвать его "application-dev.properties" (пока что содержимое можно взять из local профиля)
3. В файле "application.properties" оставить только настройки hibernate

Для изменения текущего профиля нужно будет зайти в конфигурацию проекта, нажать на "modify options" и добавить вкладку "active profiles". После добавления этой опции нужно ввести имя необходимого профиля, в нашем случае **dev** или **local**.

# Авторизация в POSTMAN и тестирование api
Скачиваем и устанавливаем Postman https://www.postman.com/downloads.
1. Получаем авторизационный токен путем написания **POST** запроса: задаем **request URL:**`http://localhost:8080/api/auth/token`, в **body** запроса указываем логин и пароль пользователя за кого хотим авторизоваться в формате **JSON**
```
{ 
...
2. В ответе нам приходит токен, копируем его и в новой вкладке пишем запрос на api который хотим протестировать. Во вклаке **Autorization** выбираем **Type:**`Bearer Token` вставляем наш токен и отправляем. Документацию по api можно посмотреть в swagger по адресу http://localhost:8080/swagger-ui/index.html.
**Время жизни токена ограничено!** По прошествии некоторого времени необходимо получать новый токен.
